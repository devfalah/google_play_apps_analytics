...
fun seeResultOfRequirements(apps:List<App>):String {
    ...
    while (true){
        ...
        print("\nYou asked me about")
        when(lineNumber){
            "1" ->results = textOfRequirements[0].split(',')[0].drop(3) + (analyzer.calculateCountAppsDevelopedByGoogle(apps)).toString() + textOfRequirements[0].split(',')[1]
            "2" ->results = textOfRequirements[1].split(',')[0].drop(3) + (analyzer.calculatePercentageOfMedicalApps(apps)).toString() + textOfRequirements[1].split(',')[1]
            "3"->results = textOfRequirements[2].split(',')[0].drop(3) + (analyzer.getOldestApp(apps)).toString()  + textOfRequirements[2].split(',')[1]
            "4"->results = textOfRequirements[3].split(',')[0].drop(3) + (analyzer.calculatePercentageOfAppsRunningOnAndroid9AndUpOnly(apps)).toString()  + textOfRequirements[3].split(',')[1]
            "5"->results = textOfRequirements[4].split(',')[0].drop(3) + (analyzer.getLargest10Apps(apps)).toString()  + textOfRequirements[4].split(',')[1]
            "6"->results = textOfRequirements[5].split(',')[0].drop(3) + (analyzer.getTop10InstalledApps(apps)).toString()  + textOfRequirements[5].split(',')[1]
